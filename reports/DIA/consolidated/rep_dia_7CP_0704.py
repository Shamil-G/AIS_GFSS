...
report_name = 'Выплаты по регионам и коду выплаты (7CP-СВбр)'
report_code = '7CP-СВбр'
...
And substr(pd.rfpm_id,1,4) in ('0704')
And pd.ridt_id In (6, 7, 8)
And pd.status In (0, 1, 2, 3, 5, 7)
And pd.pnsp_id > 0
Group By Grouping Sets((SUBSTR(coalesce(ph.rfbn_id,'ZZ'), 1, 2), b.short_name, substr(ph.rfpm_id, 1, 4)), substr(ph.rfpm_id, 1, 4))
order by 1
"""
