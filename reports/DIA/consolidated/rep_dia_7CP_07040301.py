...
report_name = 'Выплаты по регионам и коду выплаты (7CP-СВбр-301)'
report_code = '7CP-СВбр-301'

stmt_report = """
SELECT	/*+parallel(8)*/ 
		SUBSTR(coalesce(ph.rfbn_id,'ZZ'), 1, 2) reg, 
		b.short_name, 
		SUBSTR(ph.rfpm_id, 1, 8) rfpm, 
		round(AVG(ph.sum_pay), 2) sz_avg
FROM payment_history ph, 
     pnpd_document pd,
     branch b
WHERE pd.pnpd_id = ph.pnpd_id
and substr(ph.rfbn_id,1,2)||'00'=b.rfbn_id(+)  
and pd.pncp_date >= to_date(:dt_from,'YYYY-MM-DD') 
and pd.pncp_date < to_date(:dt_to,'YYYY-MM-DD') + 1
And substr(pd.rfpm_id,1,8) in ('07040301')
And pd.ridt_id In (6, 7, 8)
And pd.status In (0, 1, 2, 3, 5, 7)
And pd.pnsp_id > 0
Group By Grouping Sets((SUBSTR(coalesce(ph.rfbn_id,'ZZ'), 1, 2), b.short_name,  substr(ph.rfpm_id, 1, 8)), substr(ph.rfpm_id, 1, 8))
order by 1
"""
...
def format_worksheet(worksheet, common_format):
	...
	worksheet.write(3,2, 'Наименование', common_format)
	worksheet.write(3,3, 'Код выплаты', common_format)
	worksheet.write(3,4, 'Средняя сумма выплаты', common_format)
	...
	with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
		with connection.cursor() as cursor:
			...
			for i in range(page_num):
				# Шифр отчета
				worksheet[i].write(0, 4, report_code, title_report_code)
				worksheet[i].write(1, 4, f'Дата формирования: {now.strftime("%d.%m.%Y ")}({s_date} - {stop_time})', title_format_it)
