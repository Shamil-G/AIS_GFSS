...
report_name = 'Выплаты по коду выплаты и регионам (6CB)'
report_code = '6CB'

stmt_report = """
Select /*+parallel(8)*/ rfpm,  reg,
       Count(Unique pncd_id) cnt,
       Sum(sum_pay) sum_pay
from (  
	select /*+parallel(2)*/
           substr(d.rfpm_id, 1, 4) rfpm,
           pp.pnpt_id,
           d.pncd_id,
...
def format_worksheet(worksheet, common_format):
	...
	worksheet.write(3,1, 'Код выплаты', common_format)
	worksheet.write(3,2, 'Код региона', common_format)
	worksheet.write(3,3, 'Количество получателей', common_format)
	worksheet.write(3,4, 'Сумма выплаты', common_format)
	...
	with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
		with connection.cursor() as cursor:
			...
			for i in range(page_num):
				...
				worksheet[i].write(0, 4, f'Дата формирования: {now.strftime("%d.%m.%Y ")}({s_date} - {stop_time})', title_format_it)
