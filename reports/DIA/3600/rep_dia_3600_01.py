...
def do_report(file_name: str, date_first: str, date_last: str):
	...
	with oracledb.connect(user=report_db_user, password=report_db_password, dsn=report_db_dsn, encoding="UTF-8") as connection:
		with connection.cursor() as cursor:
			...
			cursor.execute(active_stmt, dt_from=date_first,dt_to=date_last)
...
def thread_report(file_name: str, date_first: str, date_last: str):
	import threading
	log.info(f'THREAD REPORT. DATE BETWEEN REPORT: {date_first} - {date_last}, FILE_NAME: {file_name}')
	threading.Thread(target=do_report, args=(file_name, date_first, date_last), daemon=True).start()
